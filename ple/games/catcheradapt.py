...
class Fruit(pygame.sprite.Sprite):
    ...
    def __init__(self, speed, size, SCREEN_WIDTH, SCREEN_HEIGHT, rng, good_prob=0.5, good_color_id=None):
        self.speed = speed
        self.size = size
        ...
        colors = [(255,120,120), (120,255,120)]
        if good_color_id is None:
            good_color_id = self.rng.randint(2)
        self.good_color = colors[good_color_id]
        self.bad_color = colors[1-good_color_id]


        #if self.rng.rand() < self.good_prob:
        #    self.good = True
        #else:
        #    self.good = False
...
class CatcherAdapt(base.PyGameWrapper):
    """
    Same as catcher hard, except the color corresponding
    to good and bad apples is randomized every game.
    ...
    def init(self, **kwargs):
        ...
        self.fruit = Fruit(self.fruit_fall_speed, self.fruit_size,
                           self.width, self.height, self.rng, **kwargs)
    ...
    def step(self, dt):
        ...
        if self.fruit.rect.center[1] >= self.height and self.fruit.good:
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = CatcherAdapt(width=256, height=256)
