...
class CatcherRand(base.PyGameWrapper):
    """
    Based on `Eder Santana`_'s game idea.

    .. _`Eder Santana`: https://github.com/EderSantana

    Randomized version of Catcher, where the mass of the paddle changes (essentially).
    In actuality -- we just randomize the velocity that the paddle goes when commanded
    ...
    def __init__(self, width=64, height=64, init_lives=3):
        ...
        self.width = width
        self.player_speed = self._new_player_speed()
    ...
    def _new_player_speed(self):
        return np.random.uniform(0.5, 1.0) * 0.021 * self.width
    ...
    def init(self):
        # This is the reset function (called by the superclass when reset is called)
        self.score = 0
